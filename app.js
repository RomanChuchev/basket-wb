...
/* Create missing item */
function createMissingItem(el) {
	const item = document.createElement('li');
	item.classList.add('basket__item');

	const basketInfo = createBasketInfo(el, true);
	const checkbox = createBasketCheckbox(el, true);
	const controls = createControlsItems(true);

	item.append(checkbox, basketInfo, controls);
	return item;
}

function renderBasketItem() {
	basketData.forEach((el) => basketList.append(createBasketItem(el)));
	basketData.forEach((el) => missingList.append(createMissingItem(el)));
	toggleBasketList();
}
...
function createBasketCheckbox(el, isMissing = false) {
	const checkbox = document.createElement('div');
	checkbox.classList.add('checkbox');
	checkbox.classList.add('basket__checkbox');
	if (isMissing) checkbox.classList.add('basket__checkbox_missing');
	...
	img.alt = `${el.alias}`;
	img.src = isMissing ? `${el.missingImg}` : `${el.img}`;

	label.append(input, span);
	isMissing ? checkbox.append(img) : checkbox.append(label, img);
	return checkbox;
}

function createBasketInfo(el, isMissing = false) {
	const info = document.createElement('div');
	info.classList.add('basket__info');

	const basketTitle = document.createElement('p');
	basketTitle.classList.add('basket__title');
	if (isMissing) basketTitle.classList.add('basket__title_missing');
...
function createBasketControls(el) {
	...
	const controlsItems = createControlsItems();
...
function createControlsItems(isMissing = false) {
	const controlsItems = document.createElement('div');
	controlsItems.classList.add('basket-controls__items');
	if (isMissing) controlsItems.classList.add('basket-controls__items_missing');
